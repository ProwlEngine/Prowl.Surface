...
internal unsafe sealed class X11Screen : Screen
{
    ...
    internal unsafe X11Screen()
    {
        ...
        {
            IsPrimary = true;
            Name = Xlib.DisplayString(Display) ?? "";
            Position = new();
            SizeInPixels = new(Xlib.DisplayWidth(Display, 0), Xlib.DisplayHeight(Display, 0));

            int mwidth = Xlib.DisplayWidthMM(Display, 0);
            int mheight = Xlib.DisplayHeightMM(Display, 0);
    ...
    internal unsafe X11Screen(XRRMonitorInfo monitorInfo, XRRScreenResources* resources)
    {
        lock (Lock)
        {
            ...
            Name = Xlib.GetAtomName(Display, monitorInfo.name);
            ...
            for (int o = 0; o < monitorInfo.noutput; o++)
            {
                ...
                XRROutputInfo* outputInfo = Xlib.XRRGetOutputInfo(Display, resources, (nint)output);
                ...
                XRRCrtcInfo* crtcInfo = Xlib.XRRGetCrtcInfo(Display, resources, (nint)outputInfo->crtc);
