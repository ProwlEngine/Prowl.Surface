...
internal class X11Screens : IScreenImpl
{
    ...
    private class Randr15ScreensImpl : IX11Screens
    {
        ...
        private unsafe Vector2? GetPhysicalMonitorSizeFromEDID(IntPtr rrOutput)
        {
            ...
            if (width == 0 && height == 0)
                return null;
            return new Vector2(width * 10, height * 10);
        }
...
internal class X11Screen
{
    ...
    public static double GuessPixelDensity(PixelRect pixel, Vector2 physical)
    {
        var calculatedDensity = 1d;
        if (physical.x > 0)
            calculatedDensity = pixel.Width <= FullHDWidth ? 1 : Math.Max(1, pixel.Width / physical.x * 25.4 / 96);
        else if (physical.y > 0)
            calculatedDensity = pixel.Height <= FullHDHeight ? 1 : Math.Max(1, pixel.Height / physical.y * 25.4 / 96);
