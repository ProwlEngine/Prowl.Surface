            ...
            case WindowsMessage.WM_MOUSEWHEEL:
                {
                    ...
                    e = new RawMouseWheelEventArgs(
                        ...
                        new Vector2(0, (ToInt32(wParam) >> 16) / wheelDelta),
                        GetMouseModifiers(wParam));
                    break;
                }
            ...
            case WindowsMessage.WM_MOUSEHWHEEL:
                {
                    ...
                    e = new RawMouseWheelEventArgs(
                        ...
                        Owner,
                        PointToClient(PointFromLParam(lParam)),
                        new Vector2(-(ToInt32(wParam) >> 16) / wheelDelta, 0),
                        GetMouseModifiers(wParam));
                    break;
                }
...
#pragma warning restore CS8509
            ...
            case WindowsMessage.WM_SIZE:
                {
                    ...
                    {
                        var clientSize = new Vector2(ToInt32(lParam) & 0xffff, ToInt32(lParam) >> 16);
            ...
            case WindowsMessage.WM_GETMINMAXINFO:
                {
                    ...
                    if (_minSize.x > 0)
                    {
                        mmi.ptMinTrackSize.X =
                            (int)((_minSize.x * RenderScaling) + BorderThickness.Left + BorderThickness.Right);
                    }

                    if (_minSize.y > 0)
                    {
                        mmi.ptMinTrackSize.Y =
                            (int)((_minSize.y * RenderScaling) + BorderThickness.Top + BorderThickness.Bottom);
                    }

                    if (!double.IsInfinity(_maxSize.x) && _maxSize.x > 0)
                    {
                        mmi.ptMaxTrackSize.X =
                            (int)((_maxSize.x * RenderScaling) + BorderThickness.Left + BorderThickness.Right);
                    }

                    if (!double.IsInfinity(_maxSize.y) && _maxSize.y > 0)
                    {
                        mmi.ptMaxTrackSize.Y =
                            (int)((_maxSize.y * RenderScaling) + BorderThickness.Top + BorderThickness.Bottom);
                    }
    ...
    private Vector2 DipFromLParam(IntPtr lParam)
    {
        return new Vector2((short)(ToInt32(lParam) & 0xffff), (short)(ToInt32(lParam) >> 16)) / RenderScaling;
    }
